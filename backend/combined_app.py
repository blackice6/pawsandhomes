...
def signup():
    if request.method == "POST":
        ...
        # establish a connection between python flask and mysql database
        connection = pymysql.connect(host="mysql-blackice6.alwaysdata.net", user="blackice6", password="modcom1234", database="blackice6_sokogarden")
...
def signin():
    try:
        if request.method == "POST":
            # extract the email and password only
            ...
            # if there is no record it means the password and email are incorrect
            if user is None:
                ...
                # Include both token and user in the response
                return jsonify({
...
def add_product():
    if request.method == "POST":
        ...
        filename = str(int(time.time())) + "_" + product_photo.filename
...
def mpesa_payment():
    if request.method == 'POST':
        ...
        print("This is the final response: ",response.text)
